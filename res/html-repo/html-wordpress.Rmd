---
_title: "HTML Cheat Sheet for Wordpress"
author: "Lucien Baumgartner"
date: "9/26/2018"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, tidy=T)
```

```{css echo=F}
pre code, pre, code {
  white-space: pre !important;
  overflow-x: scroll !important;
  word-break: keep-all !important;
  word-wrap: initial !important;
}

pre.html{
  background-color:black;
  color:white;
}
```



# Add caption to imported pictures

The HTML-side of things when you import a picture from the wordpress media library (make sure you upload your picture first, of course) looks like this:


<img class="alignnone size-full wp-image-16090" src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />

```{html}
<img class="alignnone size-full wp-image-16090" src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />

```

&nbsp;

If you want to add a caption, we just use `figure`-wrapper and add a `figcaption`-subelement:

```{html}
<figure>
    <img class="alignnone size-full wp-image-16090" src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />
    <figcaption>Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>
```

<figure>
    <img class="alignnone size-full wp-image-16090" src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />
    <figcaption>Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>

&nbsp;

Of course, you can adjust the properties of the caption individually. Often, a caption is slightly smaller than the text, grey-ish, and centered below the image. In order to achieve this we can add the following style properties to the `figcaption`-subelement:

```{html}
style="font-size: 12px; color: grey; text-align: center"

```

So that it looks like this:

```{html}
<figure>
    <img class="alignnone size-full wp-image-16090"
     src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />
    <figcaption style="font-size: 8px; color: grey; text-align: center">Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>
```

<figure>
    <img class="alignnone size-full wp-image-16090"
     src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />
    <figcaption style="font-size: 8px; color: grey; text-align: center">Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>

&nbsp;


### dump
As ypu can see, now the caption is centered in the middle of the page, but the actual image is not. This can be changed by adding the following spec to the `img`-element:

```{html}
<figure>
    <img class="alignnone size-full wp-image-16090" style="display: block; margin-left: auto;margin-right: auto;
    width: 50%;"
     src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="1000" height="400" />
    <figcaption style="font-size: 10px; color: grey;">Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>
```

<figure>
    <img class="alignnone size-full wp-image-16090" style="display: block; margin-left: auto;margin-right: auto;
    width: 100%;"
     src="http://pwiweb.uzh.ch/wordpress/wp-content/uploads/2018/05/insta.jpg" alt="" width="800" height="400" />
    <figcaption style="font-size: 10px; color: grey;">Hey I'm a caption, my author is :: anonymous ::</figcaption>
</figure>
