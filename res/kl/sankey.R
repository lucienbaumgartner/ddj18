...
aggr <- df %>% 
  filter(!is.na(weglandhistlang)&!is.na(alterv05kurz)) %>%
  group_by(weglandhistlang) %>%
  summarise(n_move=n()) %>%
  mutate(p_move=n_move/sum(n_move)) %>% 
  ungroup %>%
  filter(!weglandhistlang=='Unzuteilbar') %>% 
  rbind(tibble(weglandhistlang='Zürich', n_move=sum(.$n_move), p_move=sum(.$p_move))) %>% 
  arrange(desc(n_move)) %>%
  mutate(weglandhistlang=factor(weglandhistlang, levels=weglandhistlang)) %>% 
  print(n=40) 
levels(aggr$weglandhistlang)
...
p <- plot_ly(
  ...
  node = list(
    label = c(levels(aggr$weglandhistlang)[1], levels(aggr$weglandhistlang[-1])), # bucket labels
    color = rep('#262C46', length(aggr$weglandhistlang)), # same color for all of them
  ...
  # I'll explain it
  link = list(
    source = rep(as.numeric(aggr$weglandhistlang)[1], 24),
    target = as.numeric(aggr$weglandhistlang)[-1],
    value =  aggr$p_move[-1]*100
